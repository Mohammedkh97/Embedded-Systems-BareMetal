...
u16 UART_SendFrame_SW(u8 data)
{
	...
	// Parity Check Bit:
	if (counterSetBits(data) == EVEN && (GET_BIT(UCSRC , UPM0) == 0 && GET_BIT(UCSRC , UPM1) == 1))
	{
		// Frame = 00000101 0
		Frame = Frame | (EVEN << (Data_Length + Start_Bit)) ;
		Parityflag = 1;
	}
	else if (counterSetBits(data) == ODD && (GET_BIT(UCSRC , UPM0) == 1 && GET_BIT(UCSRC , UPM1) == 1))
	{
