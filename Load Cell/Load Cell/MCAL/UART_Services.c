...
u16 UART_SendFrame_SW(u8 data)
{
	...
	// Parity Check Bit:
	if (counterSetBits(data) == EVEN && (GET_BIT(UCSR0C , UPM00) == 0 && GET_BIT(UCSR0C , UPM01) == 1))  // the Second part can be deleted as you can set config file .h and .c
	{
		// Frame = 00000101 0
		Frame = Frame | (EVEN << (Data_Length + Start_Bit)) ;
		Parityflag = 1;
	}
	else if (counterSetBits(data) == ODD && (GET_BIT(UCSR0C , UPM00) == 1 && GET_BIT(UCSR0C , UPM01) == 1)) // the Second part can be deleted as you can set config file .h and .c
	{
